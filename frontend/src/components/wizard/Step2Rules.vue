...
const saveRules = async () => {
  ...
  try {
    ...
    if (res.ok) {
      saveStatus.value = { success: true, message: '规则同步成功。' }
    } else {
      saveStatus.value = { success: false, message: '规则同步失败。' }
    }
  } catch (error) {
    saveStatus.value = { success: false, message: '同步过程中发生网络错误。' }
  } finally {
    loading.value = false
    setTimeout(() => {
...
<template>
  <div class="max-w-4xl mx-auto space-y-8 animate-fade-in">
    <header class="flex flex-col md:flex-row md:justify-between md:items-end border-b border-white/10 pb-6 gap-4">
      <div>
        <h1 class="text-3xl font-bold tracking-widest text-white mb-2 uppercase">偏好设置</h1>
        <p class="text-white/50 font-mono text-sm">定义你的红绿规则（用逗号分隔）</p>
      </div>
      ...
      <div class="liquid-glass p-8 rounded-3xl relative overflow-hidden group border-[rgba(0,255,100,0.2)] hover:border-[rgba(0,255,100,0.4)] transition-colors duration-500">
        ...
        <div class="relative z-10">
          <div class="flex items-center gap-3 mb-6">
            ...
            <h2 class="text-2xl font-bold text-green-400 tracking-wider text-shadow-sm shadow-green-400/50">偏好 (Like)</h2>
          </div>
          <p class="text-white/50 text-sm mb-4 font-mono">用逗号分隔的关键字，表示你喜欢的元素。</p>
          <textarea
            v-model="likeRules"
            rows="8"
      ...
      <div class="liquid-glass p-8 rounded-3xl relative overflow-hidden group border-[rgba(255,84,73,0.2)] hover:border-[rgba(255,84,73,0.4)] transition-colors duration-500">
        ...
        <div class="relative z-10">
          <div class="flex items-center gap-3 mb-6">
            ...
            <h2 class="text-2xl font-bold text-neon-primary tracking-wider text-shadow-sm shadow-red-500/50">雷区 (Avoid)</h2>
          </div>
          <p class="text-white/50 text-sm mb-4 font-mono">用逗号分隔的关键字，表示你想避开的元素。</p>
          <textarea
            v-model="avoidRules"
            rows="8"
