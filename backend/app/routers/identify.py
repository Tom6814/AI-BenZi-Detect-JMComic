...
你的任务是根据提供的作品信息，分析作品中是否包含用户指定的【避雷清单】（avoid）和【喜欢清单】（like）中的成分。
...
返回的 JSON 必须严格符合以下结构：
{
  ...
  "reasoning": "整体分析和总结，必须为 Markdown 格式。开头的第一句话必须带有强烈的情感、口语化且激进（例如：“是纯爱，猛冲！”、“核弹级雷区，快跑！”或“成分复杂，谨慎驾驶”），随后分点详细列出你的推理依据（如画风与作者、剧情推测等）。"
}
"""
...
# Task Workflow
1. **信息审视**：仔细分析提供的漫画标题、简介、标签和评论。
2. **深度挖掘（联网/知识库）**：如果仅凭当前信息存在歧义，或者该漫画是已有一定知名度的作品，请务必利用网络搜索或你的内部知识库检索作者（{req.author}）的其他作品风格，获取更详尽的剧情走向、同人设定或读者排雷反馈。
3. **精准匹配**：将漫画的实际内容与用户提供的【避雷清单】和【喜欢清单】进行逐一、严格的比对。
4. **格式化输出**：严格按照 System Prompt 的 JSON 格式输出最终结论，不要输出任何 JSON 之外的问候语或解释性纯文本。

# Input Data
- 标题：{req.title}（可能是生肉或汉化名，请综合推测）
- 作者：{req.author}（如为多作者请综合考虑，必须利用知识库检索其一贯风格）
- 标签：{', '.join(req.tags)}
- 简介：{req.description}
- 封面URL：{req.cover_url}
- 评论区精选（点赞数最高的前10条，已自动过滤无关信息）：
