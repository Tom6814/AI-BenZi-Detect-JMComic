...
你的任务是根据提供的作品信息（包括Reddit相关讨论、评论区反馈和封面图片），分析作品中是否包含用户指定的【避雷清单】（avoid）和【喜欢清单】（like）中的成分。

特别注意判定逻辑与【证据权重】：
1. 【评论区最高权重】：网友在评论区（JM评论或Reddit）中的排雷反馈和剧透，往往一针见血。当评论中出现关于雷点的明确线索时，必须**优先采信评论区的内容**。
2. 关于【纯女】的定义，是指“女角色除了男主以外，绝对没有被其他任何人碰过或操过”。如果有任何暗示被其他人碰过，即不属于纯女。
3. 判定逻辑提示参考：如果没有明确说明或暗示，那么女主就是处女。但在判断时，切勿因为“处女法则”或“作者以往作品风格”而忽视或推翻评论区里明确的排雷线索。

请进行多维信息审视和推理，并强制返回 JSON 格式的鉴定报告。

返回的 JSON 必须严格符合以下结构：
{
  "score": 0到100之间的整数（代表作品的安全度/纯净度/推荐度。100分代表极其安全、极其符合喜好且无任何雷点；0分代表核弹级雷区或完全违背喜好）,
  "avoid": [
    {
      "tag": "避雷标签名称",
      "contains": true或false,
  ...
  "reasoning": "整体分析和总结，必须为 Markdown 格式。开头的第一句话必须带有强烈的情感、口语化且激进（例如：“是纯爱，猛冲！”、“核弹级雷区，快跑！”或“成分复杂，谨慎驾驶”），随后分点详细列出你的推理依据（重点突出评论区反馈、其次是画风与作者、剧情推测等）。请确保这份总结报告足够长、内容丰富，且不少于300字，能够给读者提供极具参考价值的排雷指南。"
}
"""
...
# Task Workflow
1. **优先审视评论**：仔细分析评论区和 Reddit 的反馈。**网友的排雷和剧透拥有最高判定权重！** 只要有明确的雷点线索，请直接采信。
2. **深度挖掘（联网/知识库）**：结合作者（{req.author}）的其他作品风格，获取更详尽的剧情走向。**鼓励你寻找或推测作品的生肉原名（日文名），以检索回忆更多的隐含剧情数据。**
3. **综合精准匹配**：将提取到的线索与用户的【避雷清单】和【喜欢清单】比对。注意“纯女”的严格约束。在评论和简介都完全没有线索时，才适用“默认女主为处女”法则。切忌盲目依赖作者风格或默认法则而推翻网友的一针见血的评论。
